...
func TestIsReadOnlyUnixCommandInDirsWithoutPathContext(t *testing.T) {
	tests := []struct {
		...
		{name: "journalctl recent errors", command: `journalctl -p err..alert --since "10 minutes ago" --no-pager`, want: true},
		{name: "journalctl follow", command: `journalctl -f -p warning..alert --no-pager`, want: true},
		{name: "journalctl vacuum", command: `journalctl --vacuum-time=7d`, want: false},
		{name: "journalctl rotate", command: `journalctl --rotate`, want: false},
		{name: "journalctl flush", command: `journalctl --flush`, want: false},
