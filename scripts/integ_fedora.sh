...
LOCAL_KDE_CONFIG_DIR="${HOME}/.config"
...
usage() {
  cat <<'EOF'
Usage: scripts/integ_fedora.sh [--uninstall]

Installs or removes the Fedora desktop integration for Terminal Agent popup GUI.
EOF
}
...
ensure_fedora() {
  ...
  if [[ -r /etc/os-release ]]; then
    if ! grep -qi '^ID=fedora\|^ID_LIKE=.*fedora' /etc/os-release; then
      fail "This integration script is intended for Fedora."
    fi
  fi
}
...
ensure_build_deps() {
  local packages=()
  local pkg
  for pkg in mesa-libGL-devel libX11-devel libXcursor-devel libXrandr-devel libXinerama-devel libXi-devel libXxf86vm-devel; do
  ...
  if ((${#packages[@]} > 0)); then
    log "Installing missing GUI build dependencies via dnf"
...
configure_kde_shortcut() {
	...
	# Fedora Plasma Wayland currently works by discovering the installed desktop entry
	# in System Settings -> Keyboard -> Shortcuts. Binding the discovered application
	# entry is the verified supported path; the old _launch automation attempt is not.
...
remove_kde_shortcut() {
	...
	# No KDE shortcut automation is currently installed by this script.
	# Keep this as a no-op so uninstall remains safe for existing users.
...
uninstall_integration() {
	...
	refresh_desktop_database
	log "Removed Fedora desktop integration files"
}
...
print_summary() {
  local desktop="$1"
  log "Fedora desktop integration complete"
